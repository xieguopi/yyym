/* 余姚杨梅 · 下单区  —  checkout.jsx */
const { useState:useStateC } = React;

function Stepper({ qty, setQty, accent }){
  const btn={width:30,height:30,borderRadius:9,border:'1px solid var(--line)',background:'var(--paper)',
    color:'var(--ink)',fontSize:18,lineHeight:1,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'};
  return (
    <div style={{display:'flex',alignItems:'center',gap:0,border:'1px solid var(--line)',borderRadius:10,overflow:'hidden'}}>
      <button style={{...btn,border:'none',borderRight:'1px solid var(--line)',borderRadius:0,opacity:qty<=1?.4:1}}
      ...
      <button style={{...btn,border:'none',borderLeft:'1px solid var(--line)',borderRadius:0,color:accent}}
...
/* --------------------------------------------------------- OFFER CARD */
function Offer({ t, specId, setSpecId, qty, setQty }){
  const benefits=[
    { ic:'truck',  k:'顺丰空运 次日达' },
    ...
    <section style={{padding:'36px 18px 26px',background:'var(--paper-2)'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 6px',marginBottom:16}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:6,background:'rgba(167,47,53,.08)',
          ...
          <span style={{width:7,height:7,borderRadius:'50%',background:'var(--berry-bright)',
            boxShadow:'0 0 0 0 rgba(194,67,74,.6)',animation:'pulse 1.6s infinite'}}></span>
          <span style={{fontSize:11,color:'var(--berry)',fontWeight:600,whiteSpace:'nowrap'}}>今日剩 38 份</span>
        </div>
      </div>
      ...
      <div style={{background:'var(--paper)',borderRadius:20,border:'1px solid var(--line)',overflow:'hidden',
        ...
        <div style={{position:'relative',height:200}}>
          <img src="assets/img-cluster.jpg" alt="精品杨梅礼盒" style={{width:'100%',height:'100%',objectFit:'cover',display:'block'}}/>
          <div style={{position:'absolute',top:12,left:12,display:'flex',gap:7}}>
            ...
            <span style={{background:'rgba(255,255,255,.9)',color:'var(--berry-deep)',fontSize:10.5,fontWeight:600,
              padding:'4px 9px',borderRadius:8,whiteSpace:'nowrap'}}>地理标志</span>
          </div>
        </div>

        <div style={{padding:'18px 16px 18px'}}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:6}}>
            <div style={{display:'flex',gap:1}}>{[0,1,2,3,4].map(i=><Ico.star key={i} s={13} f="var(--gold)"/>)}</div>
            <span style={{fontSize:11,color:'var(--ink-faint)'}}>4.9 · 2.3 万人已预定</span>
          </div>
          <h3 style={{margin:'0 0 14px',fontFamily:'var(--serif)',fontSize:19,fontWeight:600,color:'var(--ink)'}}>四明山·精品杨梅礼盒</h3>
            ...
            {SPECS.map(s=>{
              ...
              return (
                <button key={s.id} onClick={()=>setSpecId(s.id)} style={{textAlign:'left',cursor:'pointer',
                  ...
                  border:'1.5px solid '+(on?t.accent:'var(--line)'),
                  ...
                  <div>
                    <div style={{display:'flex',alignItems:'center',gap:7,marginBottom:2}}>
                      ...
                      <span style={{fontSize:10,color:on?'#fff':'var(--ink-soft)',background:on?t.accent:'var(--paper-3)',
                        padding:'1px 7px',borderRadius:6}}>{s.weight}</span>
                    </div>
                    <div style={{fontSize:10.5,color:'var(--ink-faint)',whiteSpace:'nowrap'}}>{s.note}</div>
                  </div>
                  <div style={{textAlign:'right',flexShrink:0}}>
                    <span style={{fontFamily:'var(--serif)',fontSize:18,fontWeight:700,color:on?t.accent:'var(--ink)'}}>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:18}}>
            ...
            <Stepper qty={qty} setQty={setQty} accent={t.accent}/>
          </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:11,paddingTop:16,borderTop:'1px dashed var(--line)'}}>
            {benefits.map((b,i)=>(
              <div key={i} style={{display:'flex',alignItems:'center',gap:11}}>
                <span style={{color:t.accent}}>{Ico[b.ic]({s:18,c:t.accent})}</span>
...
Object.assign(window, { Offer, Stepper });
